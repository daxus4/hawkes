...
HAWKES_DIRECTORY_PATH = '/home/davide/Desktop/phd/hawkes/data_090min_training_logged/'
...
def get_hawkes_results(
    ...
    for i in range(simulation_period_duration_seconds):
        ...
        current_attack_times_index = (attack_times >= start_warm_up_period) & (attack_times < end_warm_up_period)
        current_attack_times = attack_times[current_attack_times_index]
        current_attack_times_with_start = np.concatenate([[start_warm_up_period], current_attack_times])

        current_log_distanced_attack_times = get_logarithmic_distanced_events(
            current_attack_times_with_start, offset=4
        )[1:]

        current_attack_times = current_attack_times - start_warm_up_period
        current_log_distanced_attack_times = current_log_distanced_attack_times - start_warm_up_period

        real_next_event_timestamp = attack_times[attack_times > end_warm_up_period][0] - start_warm_up_period

        log_distanced_end_time = np.log10(
            warm_up_period_duration_seconds - current_attack_times[-1]
        ) + 4 + current_log_distanced_attack_times[-1]

        log_distanced_t_max_simulation = np.log10(
            warm_up_period_duration_seconds + prediction_period_duration_seconds - current_attack_times[-1]
        ) + 4 + current_log_distanced_attack_times[-1]

        sim_hawkes = get_hawkes_simulation(
            current_log_distanced_attack_times, log_distanced_end_time, log_distanced_t_max_simulation,
            abb, baa, caa
        )

        log_distanced_predicted_timestamps = get_predicted_timestamps(sim_hawkes, log_distanced_end_time)
        predicted_timestamps = 10 ** (log_distanced_predicted_timestamps - current_log_distanced_attack_times[-1] - 4) + attack_times[-1]
...
def get_hawkes_simulation(
    attack_times: np.ndarray, end_time: float, t_max_simulation: float,
    abb: float, baa: List[np.ndarray], caa:np.ndarray, seed: int = 1039
) -> hk.SimuHawkesExpKernels:
    sim_hawkes = hk.SimuHawkesExpKernels(
        adjacency=caa, decays=baa, baseline=[abb], end_time=end_time, seed=seed
    )
    sim_hawkes.track_intensity(1)

    sim_hawkes.set_timestamps([attack_times], end_time)
...
def get_log_distanced_training_attack_times(attack_times: np.ndarray, start_time_training: float, end_time_training: float) -> np.ndarray:
    return get_logarithmic_distanced_events(
        attack_times[(attack_times >= start_time_training) & (attack_times <= end_time_training)]
    )
    ...
    for filename, densities_info in file_densities_map.items():
        ...
        for density_info in densities_info:
            ...
            try:
                ...
                log_distanced_attack_times = get_logarithmic_distanced_events(attack_times, offset=4)
                ...
                log_distanced_training_attack_times = get_log_distanced_training_attack_times(
                    log_distanced_attack_times, start_time_training_timestamp, start_time_simulation_timestamp
                )

                best_decays = get_best_decays_with_cross_validation(log_distanced_training_attack_times)
                abb, baa, caa = get_hawkes_parameters_trained(log_distanced_training_attack_times, best_decays)
